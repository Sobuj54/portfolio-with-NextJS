...
const FeaturedArticle = ({ img, title, time, summary, link }) => {
  return (
    <li className="col-span-1 border border-solid w-full p-4 bg-light border-dark rounded-2xl relative">
...
const MovingImg = ({ title, img, link }) => {
  ...
  return (
    <Link
      ...
      <h3 className="text-lg capitalize font-semibold hover:underline">
        {title}
      </h3>
      <FramerImage
        ref={imgRef}
...
const Article = ({ img, title, date, link }) => {
  return (
    <motion.li
      ...
      viewport={{ once: true }}
      className="w-full relative p-4 py-6 my-4 rounded-xl flex items-center justify-between bg-light text-dark first:mt-0 border border-solid border-dark
    ...
    <>
      ...
      <main className="w-full mb-16 flex flex-col items-center justify-center overflow-hidden">
        <Layout className="pt-16">
          ...
          <h2 className="text-4xl font-bold text-center my-16 mt-32 w-full">
            All Articles
          </h2>
          <ul>
            {/* article 1 */}
